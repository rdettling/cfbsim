...
Calculated Prestige is clamped to each program's catalog bounds.
...
move the final tier distribution away from those raw percentages.
[Program Prestige](../systems/program-prestige.md) owns the calculation and
bounds behavior.
...
- Every team belongs to exactly one conference or the `independents` array.
- Conference `teams` and `independents` are exact arrays of canonical program
  names; duplicate and unknown names are rejected.
...
The builder walks completed seasons chronologically. Before adding a season's
result, it calculates the active field's starting Prestige from the already-
built prior history and records that tier as a display snapshot. A season's own
finish therefore never affects its historical starting tier.
...
```text
npm run data:build
npm run data:build -- --scope seasons
npm run data:build -- --scope odds
npm run data:build -- --scope historical-games
```
...
renames. The by-team directory is staged and replaced as a whole. Every writer
compares expected bytes first and leaves an unchanged file or directory
untouched.

Omitting `--scope` builds all projections. `seasons` builds only the season
index and compact history, `odds` builds only betting odds, and
`historical-games` builds only its index and by-team projections. Unknown,
repeated, or combined scopes fail.

### Check

```text
npm run data:check
npm run data:check -- --scope seasons
npm run data:check -- --scope odds
npm run data:check -- --scope historical-games
```
...
The check scopes mirror the build scopes and validate only the selected output
plus its canonical dependencies. The no-scope command remains the final
...
2. Run the affected scoped build and check.
3. Run the complete `npm run data:check` acceptance gate.
...
2. Create `public/data/seasons/YYYY.json` with the new year, exact topology,
   playoff configuration, and `results: null`.
...
| --- | --- |
| Canonical catalog or season topology | Scoped season build/check, full `data:check`, relevant league tests, `typecheck` |
| Season results | Single-year CFBD `--check`, scoped season build/check, full `data:check`, history/league tests |
| Historical games | Transform tests, scoped historical-games build/check, full `data:check`, schedule/game loader tests |
| Validator or runtime loader | Validator and `src/db/baseData.test.ts`, `data:check`, `typecheck` |
| Generator logic or simulation odds | Generator tests, two clean scoped builds, full `data:check`, affected simulation tests |
...
memory. It remains offline and write-free. Use the relevant scoped check while
iterating, then run the complete check before acceptance.
