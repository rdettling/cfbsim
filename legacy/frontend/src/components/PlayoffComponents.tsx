...
import { PlayoffTeam, BubbleTeam, ConferenceChampion } from '../interfaces';

// Playoff Settings component
export const PlayoffSettings = ({ settings }: { settings: { teams: number; autobids: number; conf_champ_top_4: boolean } }) => (
    <Box sx={{ 
        display: 'flex', 
...
// Playoff Teams List component
export const PlayoffTeamsList = ({ teams, onTeamClick }: { teams: PlayoffTeam[]; onTeamClick: (name: string) => void }) => (
...
// Bubble Teams List component
export const BubbleTeamsList = ({ teams, onTeamClick }: { teams: BubbleTeam[]; onTeamClick: (name: string) => void }) => (
    <Paper sx={{ p: 2.5, borderRadius: 2, boxShadow: 1 }}>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', mb: 1.5, fontSize: '1.1rem' }}>
            Bubble Teams
        </Typography>
...
// Conference Champions List component
export const ConferenceChampionsList = ({ champions, onTeamClick }: { champions: ConferenceChampion[]; onTeamClick: (name: string) => void }) => (
    <Paper sx={{ p: 2.5, borderRadius: 2, boxShadow: 1 }}>
        <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', mb: 1.5, fontSize: '1.1rem' }}>
...
export const TeamBox = ({ team, seed, score, isWinner = false, isTBD = false, onTeamClick }: {
    team?: string;
    seed?: number;
    score?: number | null;
    isWinner?: boolean;
    ...
    const handleTeamClick = (e: React.MouseEvent) => {
        e.stopPropagation(); // Prevent matchup click from firing
        onTeamClick(displayTeam || '');
    };
...
// Matchup component
export const Matchup = ({ matchup, direction, onTeamClick }: {
    matchup: any;
    direction: 'left' | 'right';
    onTeamClick: (name: string) => void;
}) => {
    // Log matchup data to help debug
    console.log('Matchup data:', matchup);
    
    // Check if this is a real game with results
    const hasResults = matchup.score1 !== undefined && matchup.score1 !== null || 
...
export const Championship = ({ championship, onTeamClick }: { 
    championship: any;
    onTeamClick: (name: string) => void;
}) => {
    // Log championship data to help debug
    console.log('Championship data:', championship);
    
    // Check if this is a real game with results
    const hasResults = championship?.score1 !== undefined && championship?.score1 !== null || 
...
export const BracketRound = ({ title, matchups, direction = 'left', centerAlign = false, onTeamClick }: {
    title: string;
    matchups: any[];
    direction?: 'left' | 'right';
    centerAlign?: boolean;
