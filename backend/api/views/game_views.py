...
def calculate_team_stats_with_rankings(info, team_a, team_b):
    ...
    offensive_rankings = sorted(
        all_team_stats.values(), key=lambda x: x["offensive_ypg"], reverse=True
    )
    offensive_rank_map = {
        team_stat["team"].id: idx + 1
        for idx, team_stat in enumerate(offensive_rankings)
    }

    # Defensive yards per game (lower is better)
    defensive_rankings = sorted(
        all_team_stats.values(), key=lambda x: x["defensive_ypg"]
    )
    defensive_rank_map = {
        team_stat["team"].id: idx + 1
        for idx, team_stat in enumerate(defensive_rankings)
    }

    # Points per game (higher is better)
    points_rankings = sorted(
        all_team_stats.values(), key=lambda x: x["points_per_game"], reverse=True
    )
    points_rank_map = {
        team_stat["team"].id: idx + 1 for idx, team_stat in enumerate(points_rankings)
    }
