...
def simGame(game):
    ...
    for i in range(drivesPerTeam * 2):
        if i == 0 or i == drivesPerTeam:
            fieldPosition = 20
        if i % 2 == 0:
            drive, fieldPosition = simDrive(game, i, fieldPosition, game.teamA, game.teamB)
            ...
            drive, fieldPosition = simDrive(game, i, fieldPosition, game.teamB, game.teamA)
...
def getSpread(gap, tax_factor=0.05, one=False):
    if not one:
        odds = {}
        ...
        for i in range(gap + 1):
            ...
            spread = round((results['scoreA'] - results['scoreB']) * 2) / 2  # round to nearest half-point
            ...
            implied_probA = results['winA'] + tax_factor / 2
            implied_probB = results['winB'] + tax_factor / 2
