...
import { finalizeGameSimulation, prepareInteractiveLiveGame } from '../../domain/sim';
...
import { buildDriveUi, buildNextHeader, mapPlayRecord, resolveDecision } from '../../domain/sim/ui';
...
export const useLiveGameSim = ({
  ...
  const buildDecisionPrompt = (state: InteractiveDriveState): DecisionPrompt => ({
  ...
  const updateDecisionPrompt = (driveState: InteractiveDriveState | null) => {
    ...
    const isUserOffense = Boolean(context.userTeamId) && context.currentOffense?.id === context.userTeamId;
  ...
  const addPlaysToDrive = (driveRecord: DriveRecord, newPlays: PlayRecord[], finalizeDriveUi = false) => {
  ...
  const advanceToNextDrive = async () => {
    ...
    const isTeamA = context.inOvertime ? context.otPossession === 0 : context.nextOffenseIsTeamA;
    ...
    const fieldPosition = context.inOvertime ? OT_START_YARD_LINE : context.fieldPosition;
    ...
    const driveState = startInteractiveDrive(simContext, fieldPosition, context.driveNum);
  ...
  const finalizeDrive = async (
    ...
    if (context.inOvertime) {
      ...
      context.nextOffenseIsTeamA = halftimeReached ? !context.openingIsTeamA : !context.nextOffenseIsTeamA;
    }
  ...
  const handleDecision = async (decision: 'run' | 'pass' | 'punt' | 'field_goal') => {
    const context = contextRef.current;
  ...
  const simulateAutoPlays = async (count: number) => {
    ...
    while (remaining > 0) {
      ...
      const stepResult = stepInteractiveDrive(simContext, context.currentDriveState, 'auto', !context.inOvertime);
      await applyStepResult(stepResult);
  ...
  const simulateAutoDrive = async () => {
    ...
    while (guard < 200) {
      ...
      stepResult = stepInteractiveDrive(simContext, driveState, 'auto', !context.inOvertime);
  ...
  const simulateToEnd = async () => {
    ...
    let guard = 0;
    while (guard < 5000 && !context.simGame.winner) {
  ...
  const derived = (() => {
    ...
    const displayPlay = interactiveState
      ? {
          ...
          header: buildNextHeader(interactiveState.fieldPosition, interactiveState.down, interactiveState.yardsLeft),
    ...
    const isUserOffenseNow =
      allowUserDecision && Boolean(context?.currentOffense) && context?.currentOffense?.id === context?.userTeamId;

    const isTeamAOnOffense = displayDrive
      ? displayDrive.offense === gameData?.teamA.name
      : context?.currentOffense?.id === gameData?.teamA.id;

    const fieldPosition = displayPlay?.startingFP || interactiveState?.fieldPosition || 20;

    const previousPlayYards =
      allowUserDecision && interactiveState
        ? (() => {
  ...
  return {
    state: {
      ...
      isTeamAOnOffense: Boolean(derived.isTeamAOnOffense),
