...
export const RatingsStatsDesktop = ({
  data,
  onTeamClick,
}: RatingsStatsDesktopProps) => (
  <Box
    sx={{
    ...
    <Stack spacing={1.5} sx={{ minHeight: 0, overflowY: { lg: 'auto' }, pr: { lg: 0.5 } }}>
      <Paper component="section" variant="outlined">
        ...
        {data.prestige_stars_table.length > 0 ? (
            ...
            <Table size="small" aria-label="Star distribution by prestige">
              <TableHead>
                <TableRow sx={{ bgcolor: 'background.default' }}>
                  ...
                  {stars.map(star => <TableCell key={star} align="right">{star}★</TableCell>)}
                </TableRow>
              </TableHead>
              <TableBody>
                {data.prestige_stars_table.slice().reverse().map(row => (
                  <TableRow key={row.prestige} hover>
                    <TableCell><Chip label={`Tier ${row.prestige}`} size="small" variant="outlined" /></TableCell>
                    <TableCell align="right">{row.team_count}</TableCell>
                    <TableCell align="right" sx={{ fontWeight: 600 }}>{row.avg_rating.toFixed(1)}</TableCell>
                    <TableCell align="right">{row.average_stars.toFixed(2)}</TableCell>
                    {stars.map(star => (
                      <TableCell key={star} align="right">{row.star_percentages[star].toFixed(1)}%</TableCell>
                    ))}
                  </TableRow>
                ))}
              </TableBody>
          ...
          <Typography color="text.secondary" sx={{ p: 2 }}>No prestige data available.</Typography>
        )}
      </Paper>
                ...
                <TableRow key={star} hover>
                  <TableCell>
                    <Stack direction="row" spacing={0.5} alignItems="center">
                      ...
                      <Typography variant="body2" sx={{ fontWeight: 600 }}>{star} stars</Typography>
                    </Stack>
                  </TableCell>
                  <TableCell align="right">{data.total_star_counts.counts[star].toLocaleString()}</TableCell>
                  <TableCell align="right" sx={{ fontWeight: 600 }}>{data.total_star_counts.avg_ratings[star]}</TableCell>
      ...
      {data.teams.length > 0 ? (
        <TableContainer sx={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
          <Table stickyHeader size="small" aria-label="Team ratings">
            <TableHead>
              <TableRow sx={{ bgcolor: 'background.default' }}>
                <TableCell align="center" sx={{ width: 58 }}>Rank</TableCell>
            ...
            <TableBody>
              {data.teams.map((team, index) => (
                <TableRow key={team.name} hover>
                  ...
                  <TableCell align="right" sx={{ fontWeight: 600 }}>{team.rating}</TableCell>
        ...
        <Typography color="text.secondary" sx={{ p: 2 }}>No team ratings available.</Typography>
      )}
    </Paper>
  </Box>
);
