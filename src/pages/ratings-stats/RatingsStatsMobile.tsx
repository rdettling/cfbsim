...
export const RatingsStatsMobile = ({
  data,
  onTeamClick,
}: RatingsStatsMobileProps) => (
  <Stack spacing={2} sx={{ display: { xs: 'flex', md: 'none' } }}>
    <Box component="section">
      <SectionHeading title="Star Distribution by Prestige" subtitle="Player star mix by program tier" />
      {data.prestige_stars_table.length > 0 ? (
          ...
          {data.prestige_stars_table.slice().reverse().map(row => (
            <Paper key={row.prestige} variant="outlined" sx={{ p: 1.25 }}>
              <Stack direction="row" alignItems="center" justifyContent="space-between">
                ...
                <Typography variant="body2" color="text.secondary">{row.team_count} teams</Typography>
              </Stack>
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, mt: 1 }}>
                <Box>
                  ...
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>{row.avg_rating.toFixed(1)}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary">Avg stars</Typography>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>{row.average_stars.toFixed(2)}</Typography>
                </Box>
              </Box>
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 0.5, mt: 1 }}>
                {stars.map(star => (
                  <Box key={star} sx={{ p: 0.5, bgcolor: 'action.hover', borderRadius: 1, textAlign: 'center' }}>
        ...
        <Paper variant="outlined" sx={{ p: 2 }}><Typography color="text.secondary">No prestige data available.</Typography></Paper>
      )}
    </Box>
    ...
    <Box component="section">
      ...
      <Paper variant="outlined" sx={{ overflow: 'hidden' }}>
        {stars.map((star, index) => (
          <Box
            key={star}
            sx={{ p: 1.25, borderBottom: index === stars.length - 1 ? 0 : '1px solid', borderColor: 'divider' }}
          >
            <Stack direction="row" justifyContent="space-between" alignItems="center">
              ...
              <Typography variant="body2">{data.total_star_counts.counts[star].toLocaleString()} players</Typography>
            </Stack>
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 0.5, mt: 1 }}>
              {[
    ...
    <Box component="section">
      ...
      {data.teams.length > 0 ? (
        <Paper variant="outlined" sx={{ overflow: 'hidden' }}>
          {data.teams.map((team, index) => (
            <Stack
              ...
              alignItems="center"
              spacing={1}
              sx={{ p: 1.25, borderBottom: index === data.teams.length - 1 ? 0 : '1px solid', borderColor: 'divider' }}
            >
              <Typography sx={{ width: 28, textAlign: 'center', fontWeight: 600 }}>{index + 1}</Typography>
              <TeamLogo name={team.name} size={32} />
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <TeamLink name={team.name} onTeamClick={onTeamClick} />
                <Typography variant="caption" color="text.secondary" display="block">Tier {team.prestige}</Typography>
              </Box>
        ...
        <Paper variant="outlined" sx={{ p: 2 }}><Typography color="text.secondary">No team ratings available.</Typography></Paper>
      )}
    </Box>
  </Stack>
);
