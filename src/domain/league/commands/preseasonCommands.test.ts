...
import { loadHomeData } from '../loaders/season/loadHomeData';
import { loadNewLeagueData } from '../loaders/season/loadNewLeagueData';
import { loadPlayer } from '../loaders/team/loadPlayer';
import { loadDashboard } from '../loaders/season/loadDashboard';
import { loadNonCon } from '../loaders/season/loadNonCon';
import { listAvailableOpponents } from '../loaders/season/listAvailableOpponents';
import { loadTeamSchedule } from '../loaders/season/loadTeamSchedule';
import { loadWeekSchedule } from '../loaders/season/loadWeekSchedule';
import { loadGame } from '../loaders/season/loadGame';
import { startNewLeague } from './startNewLeague';
import { scheduleNonConGame } from './scheduleNonConGame';
import {
  dismissPendingRivalry,
  removePreseasonGame,
} from './preseasonScheduleRemoval';
...
describe('startNewLeague', () => {
  ...
  it('locks an accepted fixed rivalry and allows manual rescue of an omitted one', async () => {
    ...
    const created = await loadNonCon();
  ...
  it('persists manual home and away sites and applies home-field odds', async () => {
    ...
    await scheduleManualGame('West 3', 3, 'Home');
    const homeResult = await loadNonCon();
    ...
    await scheduleManualGame('West 3', 4, 'Away');
    const awayResult = await loadNonCon();
  ...
  it('removes manual games and restores rivalry behavior when rescheduled', async () => {
    ...
    await scheduleRivalryGame('West 1', 4);
    const returned = await loadNonCon();
  ...
  it.each([
    ...
    async ({ teams, lastWeek }) => {
      ...
      await startNewLeague(buildInput(teams));
      const db = await getDb();
      ...
      expect((await loadNonCon()).info).toEqual(league.info);
    },
  );
  ...
  it('keeps the existing league after preparation failure and succeeds on retry', async () => {
    ...
    await expect(startNewLeague(buildInput())).resolves.toBeUndefined();
    expect((await loadNonCon()).info).toMatchObject({
      stage: 'preseason',
      team: 'Test State',
    });
  });
