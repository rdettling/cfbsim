import type { GeneratedGameNews } from '../../../src/domain/news/generate';
...
import { DECK_TEMPLATES_BY_ID } from '../../../src/domain/news/deckTemplates';
import { hasOddsUpset, hasRankingUpset } from '../../../src/domain/news/policy';
import {
...
export const ACCEPTED_NEWS_CONTENT_CHECKSUM = '0f2ab83d';
...
export const evaluateNewsAudit = (
  ...
  if (
    committedRepresentative &&
    newsContentChecksum !== ACCEPTED_NEWS_CONTENT_CHECKSUM
  ) {
    violations.push(notice(
      'news_content_drift',
      `Simulation changed news content checksum ${newsContentChecksum}; expected ${ACCEPTED_NEWS_CONTENT_CHECKSUM}.`,
    ));
  }
