...
import {
  ACCEPTED_NEWS_CONTENT_CHECKSUM,
  evaluateNewsAudit,
} from './audit';
import { storyTemplateSupportsEntry } from './validation';
import {
  generateNewsAuditCorpus,
} from './corpus';
import type {
  SeasonCorpusData,
  SeasonSimulationSnapshot,
...
describe('news editorial audit corpus', () => {
  ...
  const representativeIt = process.env.NEWS_REPRESENTATIVE === '1' ? it : it.skip;
  representativeIt('matches the accepted representative news-content contract', () => {
    const representativeConfiguration = {
      seed: 20260809,
      seeds: 3,
      seasons: 2,
      replaySeeds: 1,
    ...
    const entries = generateNewsAuditCorpus(data, representativeConfiguration);
    const summary = evaluateNewsAudit(entries, representativeConfiguration);

    expect(summary.newsContentChecksum).toBe(ACCEPTED_NEWS_CONTENT_CHECKSUM);
