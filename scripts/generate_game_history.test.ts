...
import {
  ...
  getHistoricalWeek,
  parseGenerateGameHistoryArgs,
  runGenerateGameHistory,
} from './generate_game_history';
...
import type { YearData } from '../src/types/baseData';
...
describe('game history generation', () => {
  it('accepts one explicit production year', () => {
    expect(parseGenerateGameHistoryArgs(['--year', '2025']))
      .toEqual({ year: 2025 });
    expect(() => parseGenerateGameHistoryArgs(['--preview']))
      .toThrow('Unknown');
  });
  ...
  it('builds deterministic single-season public output', async () => {
    ...
    const run = () => runGenerateGameHistory({
      options: { year: 2025 },
      rawDirectory,
      outputDirectory,
      stagingParent,
  ...
  it('fails when a selected raw snapshot is missing', async () => {
    ...
    await expect(runGenerateGameHistory({
      options: { year: 2025 },
      rawDirectory,
