...
const YEAR_DATA_DIRECTORY = join(ROOT, 'public', 'data', 'years');
...
export type GenerateGameHistoryOptions = {
  year: number | null;
};
...
export const parseGenerateGameHistoryArgs = (
  args: string[],
): GenerateGameHistoryOptions => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error(`Unknown generate:game-history argument: ${argument}`);
  }
...
const getCompletedYears = async () => {
  const index = await readJson<{ years: string[] }>(
    join(ROOT, 'public', 'data', 'years', 'index.json'),
  );
...
const buildTeamConferences = (yearData: YearData) => {
  ...
  for (const [conference, data] of Object.entries(yearData.conferences)) {
    for (const team of Object.keys(data.teams)) {
      conferences.set(canonicalHistoricalTeamName(team), conference);
    }
  }
  for (const team of Object.keys(yearData.independents)) {
    conferences.set(canonicalHistoricalTeamName(team), null);
  }
  return conferences;
};
...
const toHistoricalGame = (
  ...
  const homeTeam = canonicalHistoricalTeamName(providerHomeTeam);
  const awayTeam = canonicalHistoricalTeamName(providerAwayTeam);
  ...
  for (const raw of rawGames) {
    for (const [name, classification] of [
      ...
      if (
        classification === 'fbs' &&
        typeof name === 'string' &&
        !supportedTeams.has(canonicalHistoricalTeamName(name))
      ) {
...
export const runGenerateGameHistory = async ({
  options,
  ...
  options: GenerateGameHistoryOptions;
  rawDirectory?: string;
  ...
  for (const year of years) {
    ...
    const [regular, postseason, rankings, rawYearData] = await Promise.all([
      ...
      readJson<unknown>(join(YEAR_DATA_DIRECTORY, `${year}.json`)),
    ]);
  ...
  const generatedAt = [
    existingIndex?.generated_at,
    ...years.map(year => manifest.seasons[String(year)].fetched_at),
  ].filter((value): value is string => value !== undefined)
    .sort()
    .at(-1);
  if (!generatedAt) {
    throw new Error('Game-history generation requires at least one season.');
  }
  const index = validateHistoricalGamesIndex({
    generated_at: generatedAt,
    source: GAME_HISTORY_SOURCE,
    years: outputYears,
  } satisfies HistoricalGamesIndex);
  await writeFile(
  ...
  const teamGames = buildHistoricalGamesByTeam(outputSeasons, supportedTeams);
  await Promise.all(teamGames.map(teamData => {
    const validated = validateHistoricalGamesForTeam(
      teamData,
      teamData.team,
      new Set(outputYears),
    ) satisfies HistoricalGamesForTeam;
    return writeFile(
      join(byTeamDirectory, getHistoricalTeamGamesFileName(teamData.team)),
...
const main = async () => {
  const result = await runGenerateGameHistory({
    options: parseGenerateGameHistoryArgs(process.argv.slice(2)),
  });
